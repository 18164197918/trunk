...
export default {
  ...
  methods: {
    ...
    addhoursList () {
      if (this.checkWeekList.length === 0) {
        ...
        this.weekList.forEach((item, index) => {
          if (item.isSelected) {
            arry.push(item.label)
            if (index === 6) {
              if (arry.length === 7) this.format = '周一至周日'
              else {
                if (arry.length === 1) sty.push(`${arry[0]}`)
                else if (arry.length > 1) sty.push(`${arry[0]}至${arry[arry.length - 1]}`)
              }
            }
            if (sty.length !== 0) this.format = sty.join(',')
          } else {
            if (arry.length === 1) sty.push(`${arry[0]}`)
            else if (arry.length > 1) sty.push(`${arry[0]}至${arry[arry.length - 1]}`)
            if (index === 6) {
              if (arry.length === 5 && arry[0] === '周一' && arry[4] === '周五') this.format = '工作日'
    ...
    delhoursList (index) {
      for (let obj of this.weekList) {
        for (let i in this.p_hoursList[index].days) {
          if (obj.value === this.p_hoursList[index].days[i]) obj.isDisabled = false
        }
      }
