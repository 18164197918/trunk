  ...
  <div class="wrapper design-module-wrapper" @mousedown="dataCache.colorPickerStatus=false">
    ...
    <div class="right system-box">
      ...
      <div class="item" :class="{'on': selectedModuleIndex === 1}">
        ...
        <div class="base-block pr10">
          <div class="base-set-box">
            <div class="tit">按钮文字 <i @click="initBaseFont('AN')" class="iconfont iconfont-clear clear-setting ml5" title="还原默认设置"></i></div>
            <dl v-for="item in fontAnArr" class="el-block mb10">
              <dt>{{item.label}}</dt>
              <base-range-input :css="cssObj[item.value].css" propName="fontSize" min="12" max="100" unit="px"></base-range-input>
              <base-color-picker :css="cssObj[item.value].css" propName="color" class="w40 ml20" vc-position="top-right"></base-color-picker>
              <div class="label-btn-group ml5">
                <label class="label-btn" title="加粗" style="line-height: 24px;">
                  <input v-model="cssObj[item.value].css.fontWeight" type="checkbox" true-value="bold" false-value="normal">
                  <span>B</span>
                </label>
              </div>
            </dl>
        ...
        <div class="base-block pr10">
          <div class="base-set-box">
            <div class="tit">文本文字 <i @click="initBaseFont('WB')" class="iconfont iconfont-clear clear-setting ml5" title="还原默认设置"></i></div>
            <dl v-for="item in fontWbArr" class="el-block mb10">
              <dt>{{item.label}}</dt>
              <base-range-input :css="cssObj[item.value].css" propName="fontSize" min="12" max="100" unit="px"></base-range-input>
              <base-color-picker :css="cssObj[item.value].css" propName="color" class="w40 ml20" vc-position="top-right"></base-color-picker>
              <div class="label-btn-group ml5">
        ...
        <div class="base-block pr10">
          <div class="base-set-box">
            <div class="tit">页面文字 <i @click="initBaseFont('PAGE')" class="iconfont iconfont-clear clear-setting ml5" title="还原默认设置"></i></div>
            ...
            <dl v-for="item in fontPageArr" class="el-block mb10">
              <dt>{{item.label}}</dt>
              <base-range-input :css="cssObj[item.value].css" propName="fontSize" min="12" max="100" unit="px"></base-range-input>
              <base-color-picker :css="cssObj[item.value].css" propName="color" class="w40 ml20" vc-position="top-right"></base-color-picker>
              <div class="label-btn-group ml5">
...
<script>
  ...
  import dataSite from '../../pc&mobile/common/data-site'
  import dataPage from '../../pc&mobile/common/data-page'
  import dataTools from '../../pc&mobile/common/data-tools'
  import dataCache from '../../pc&mobile/common/data-cache'
  ...
  import baseModuleBackground from '../base-module-background/index.vue'*/
  ...
  let stat,
      baseAnFontInit   = {
        anH1: {class: '.an-h1', css: {fontSize: '36px', fontWeight: 'normal'}},
        anH2: {class: '.an-h2', css: {fontSize: '32px', fontWeight: 'normal'}},
        anH3: {class: '.an-h3', css: {fontSize: '26px', fontWeight: 'normal'}},
        anH4: {class: '.an-h4', css: {fontSize: '20px', fontWeight: 'normal'}},
        anH5: {class: '.an-h5', css: {fontSize: '16px', fontWeight: 'normal'}},
        anH6: {class: '.an-h6', css: {fontSize: '12px', fontWeight: 'normal'}},
      },
      baseWbFontInit   = {
  ...
  export default {
    ...
    methods   : {
      initBaseFont(TYPE) {
        let initObj
        if(TYPE === 'AN'){
          initObj = $.extend(true, {}, baseAnFontInit)
        }else if(TYPE === 'WB'){
          initObj = $.extend(true, {}, baseWbFontInit)
        }else if(TYPE === 'PAGE'){
          initObj = $.extend(true, {}, basePageFontInit)
        }

        for (let i in initObj) {
          this.$set(this.dataSite.cssObj, i, initObj[i])
        }
      },
      mouseoverEvt(key) {
        clearTimeout(stat)
