<template>
    <div class="pc-module-xz user-module" :class="{'hover': info.assist && info.assist.a==1, 'groupHover': info.assist && info.assist.a==2}">
        <a
                :href="dataLink || dataItem && dataItem[info.static.d__link] || info.static.link"
  ...
  export default {
    ...
    computed: {
      actualSvg() {
        ...
        return _static.svg.replace(/CLASSNAME|ASPECTRATIO|WITCHONE|SRC/g, arg => {return obj[arg]})
      },
      _link () {
        let _static = this.info.static
