    ...
    <div class="wrapper-btn">
      <div v-for="(item, key) in menuList" class="pannel-btn">
        <div @click="selectOne(key)" :class="{'on': dataCache.leftBoxSelectedIndex===key}" class="ico left-menu-icon">
          <span :class="['iconfont-' + item.ico]" class="iconfont left-menu-span mr5 fz16"></span>{{item.cname}}
        </div>
  ...
  export default {
    ...
    data() {
      return {
        dataCache,
        menuList: [
          {ico: 'module', comp: addBox, cname: '基础模块'},
          {ico: 'fenlei', comp: boxCategory, cname: '内容模板'},
          {ico: 'set', comp: boxSet, cname: '系统设置'},
