    ...
    <div class="">
        ...
        <div>
            <div class="common-tit2">
                <h4><b>内容模型</b><span class="fz12 ml20 tips"><i class="iconfont iconfont-warning mr3 mt3"></i>最多可添加20条内容模型</span></h4>
            </div>
        </div>
        <!--客户信息内容-->
        ...
        <div>
            <!--<div v-for="item in modelList">-->
                <!--{{item.grade}}-->
            <!--</div>-->
            <!--v-for="item in modelList" v-if="item.grade === 'NORMAL'"-->
...
export default {
  ...
  methods: {
    ...
    editModelField (row) { // 修改模型字段按钮
      ...
      this.$router.push({
        path: `/category/content/model/creat/${this.editModelFieldId}`
      })
    },
