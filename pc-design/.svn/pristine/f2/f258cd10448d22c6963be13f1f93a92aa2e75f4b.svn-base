...
import { getFirstElByClass } from '../common/constant'
import { findObjIndex } from '../common/data-methods'
...
export default {
  ...
  computed: {
    ...
    isSelected () {
      if (this.mode === 'children-list' || this.mode === 'children-menu' || this.mode === 'children-tabbar') {
  ...
  methods: {
    ...
    mouseDownAssist (e) {
      ...
      /**
         ...
         */
      if (!this.info.lock.r && e.target.className.indexOf('resizable-handle') !== -1) {} else if (!this.allowDrag || this.indexKey) {
        e.stopPropagation()
      }
    },
    mouseUpAssist () {
